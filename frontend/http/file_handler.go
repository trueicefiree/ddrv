...
func CreateFile(c *fiber.Ctx, db *sql.DB, disc *discord.Discord) error {
	dirId := c.Params("dirId")
	...
	chunks := make([]*discord.Chunk, 0)
	dwriter := disc.NewWriter(func(c *discord.Chunk) {
		file.Size += c.Size
...
func DownloadFile(c *fiber.Ctx, db *sql.DB, disc *discord.Discord) error {
	id := c.Params("id")
	...
	if fileRange != nil {
		...
		dreader, err := disc.NewReader(chunks, r.Start)
		if err != nil {
			return err
		}
	...
	dreader, err := disc.NewReader(chunks, 0)
	if err != nil {
		return err
	}
