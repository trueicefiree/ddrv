...
func New(addr string, db *sql.DB, disc *discord.Discord) *Server {
	...
	app.Get("/files/:id", validateParam("id"), func(ctx *fiber.Ctx) error {
		return DownloadFile(ctx, db, disc)
	})
	...
	return &Server{addr: addr, db: db, app: app, disc: disc}
