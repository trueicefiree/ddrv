...
func (f *File) Read(p []byte) (int, error) {
    ...
    if err != nil && err != io.EOF {
        log.Printf("fs op=%s name=%s error=%v", "read", f.Name(), err)
    }
    return n, err
}
...
func (f *File) ReadAt(p []byte, off int64) (int, error) {
    ...
    if err != nil && err != io.EOF {
        log.Printf("fs op=%s name=%s off=%d error=%v", "readat", f.Name(), off, err)
    }
    return n, err
}
...
func (f *File) Seek(offset int64, whence int) (int64, error) {
    ...
    if err != nil {
        log.Printf("fs op=%s name=%s offset=%d whence=%d error=%v", "seek", f.Name(), offset, whence, err)
    }
    return n, err
}
...
func (f *File) Write(p []byte) (int, error) {
    ...
    if err != nil {
        log.Printf("fs op=%s name=%s error=%v", "write", f.Name(), err)
    }
    return n, err
}
...
func (f *File) WriteAt(p []byte, off int64) (int, error) {
    ...
    if err != nil {
        log.Printf("fs op=%s name=%s error=%v", "writeat", f.Name(), err)
    }
    return n, err
}
...
func (f *File) WriteString(str string) (int, error) {
    ...
    if err != nil {
        log.Printf("fs op=%s name=%s error=%v", "writestring", f.Name(), err)
    }
    return n, err
}
