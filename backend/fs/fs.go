...
func New(db *sql.DB, mgr *ddrv.Manager) afero.Fs {
    return fslog.LoadFS(&Fs{db: db, mgr: mgr})
}
...
func (fs *Fs) Open(name string) (afero.File, error) {
    file := &File{flag: os.O_RDONLY, db: fs.db, mgr: fs.mgr}
...
func (fs *Fs) OpenFile(name string, flag int, _ os.FileMode) (afero.File, error) {
    file := &File{flag: flag, db: fs.db, mgr: fs.mgr}
